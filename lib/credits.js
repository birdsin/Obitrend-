...
//
// FREE USERS
// 3 free credits once per account — no renewal
//
// SECURITY
...
const FREE_PERIOD_SECONDS = 7 * 24 * 60 * 60; // retained for compatibility; free credits do not renew
...
export async function getOrCreateFreeCredits(userId, redis) {
  ...
  const freeUsed = freeUsedKey(safeUserId);
  ...
  const script = `
    ...
    if not balance then
      ...
      redis.call("SET", KEYS[2], tostring(ARGV[1]))
      return {tonumber(ARGV[2]), tonumber(ARGV[1])}
    end
