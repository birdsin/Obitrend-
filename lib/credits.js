...
export async function activatePro(
  ...
  const script = `
    ...
    local baseExpiry =
      math.max(now, oldExpiry)

    local newExpiry =
      baseExpiry + duration

    local newCredits =
      math.max(0, oldCredits) + addedCredits

    local newTotal =
      math.max(0, oldTotal) + addedCredits
  ...
  const proCredits =
    Math.max(
      0,
      Math.floor(
        safeNumber(result?.[0], packageInfo.credits)
      )
    );
  ...
  const proCreditsTotal =
    Math.max(
      0,
      Math.floor(
        safeNumber(result?.[1], proCredits)
      )
    );
  ...
  return {
    ...
    proCreditsTotal: proCreditsTotal,
