...
export async function activatePro(
  ...
  */
  const script = `
    ...
    redis.call("SET", KEYS[1], "active", "EX", ttl)
    redis.call("SET", KEYS[2], newExpiry, "EX", ttl)
    redis.call("SET", KEYS[3], newCredits, "EX", ttl)
    redis.call("SET", KEYS[4], newExpiry, "EX", ttl)
    redis.call("SET", KEYS[5], selectedPlan, "EX", ttl)
    redis.call("SET", KEYS[6], newTotal, "EX", ttl)

    if email ~= "" then
      redis.call("SET", KEYS[7], email, "EX", ttl)
    end

    redis.call("SET", KEYS[8], reference, "EX", ttl)

    if ARGV[7] ~= "" then
      redis.call("SET", KEYS[9], ARGV[7], "EX", ttl)
    end
