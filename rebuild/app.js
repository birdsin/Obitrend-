...
await loadSession();}catch(error){console.error("OBITREND auth error:",error);if(kind==="signin"&&/invalid login credentials|invalid credentials|invalid password/i.test(messageText(error))){const attempts=getAuthAttempts(email)+1;setAuthAttempts(email,attempts);if(attempts>=3){applyLoginBlock(email);setAuthStatus("Account sign-in is blocked after 3 incorrect passwords. Use Forgot password to reset access.","error");}else{setAuthStatus(`Incorrect password. ${3-attempts} attempt${3-attempts===1?"":"s"} remaining.`,"error");}}else setAuthStatus(safeMessage(error),"error");}finally{if(kind==="signin"){if(getAuthAttempts(email)<3)$("signInBtn").disabled=false;}else{$("signInBtn").disabled=false;$("signUpBtn").disabled=false;}}}
