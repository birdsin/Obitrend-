...
async function setupVoiceSecurity(){if(!session?.user?.id)return obSecurityStatus("Please sign in first.","error");if(!obGetSpeechRecognition())return obSecurityStatus("Voice Lock is not supported on this device or browser.","error");const phrase=window.prompt("Create your private OBITREND voice phrase. You will say this phrase to unlock.");if(!phrase?.trim())return;const button=$("setupVoiceLock");if(button)button.disabled=true;obSecurityStatus("Listening… say your voice phrase now.");obListenForVoice(async transcript=>{if(!transcript){obSecurityStatus("No voice phrase detected.","error");if(button)button.disabled=false;return;}localStorage.setItem(obSecurityKey("voiceHash"),await obHash(transcript.toLowerCase().replace(/\\s+/g," ").trim()));localStorage.setItem(obSecurityKey("voice"),"1");obSecurityStatus("Voice Lock enabled.","success");if(button)button.textContent="Enabled";},error=>{obSecurityStatus(error.message||"Unable to enable Voice Lock.","error");if(button)button.disabled=false;});}
