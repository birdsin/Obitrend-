...
function openPage(page){window.scrollTo({top:0,behavior:"instant"});if(page==="create")page="home";const valid=["home","create","gallery","video","credits","account","settings"];if(!valid.includes(page))page="home";qsa(".page").forEach(el=>el.classList.remove("active-page"));$(`page${page.charAt(0).toUpperCase()}${page.slice(1)}`).classList.add("active-page");qsa(".nav-item").forEach(el=>el.classList.toggle("active",el.dataset.page===page));const titles={home:"AI Fashion Creator",create:"Create",gallery:"Gallery",video:"Video AI",credits:"Credits",account:"Account",settings:"Settings"};$("pageTitle").textContent=titles[page];$("pageKicker").textContent=page==="home"?"OBITREND":page.toUpperCase();closeSidebar();}
...
qsa("[data-ob-upload]").forEach(b=>b.addEventListener("click",()=>input.click()));qsa("[data-ob-generate]").forEach(b=>b.addEventListener("click",handleCreative));button?.addEventListener("click",handleCreative);$("obProfileButton")?.addEventListener("click",()=>openPage("account"));$("obMenuButton")?.addEventListener("click",()=>{const menu=["home","create","gallery","video","credits","account","settings"];const current=document.querySelector(".page.active-page")?.id?.replace(/^page/,"").toLowerCase()||"home";const next=menu[(menu.indexOf(current)+1)%menu.length];openPage(next);});renderRecentCreations();}
