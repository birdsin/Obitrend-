...
async function recoverMalformedGenerationCredit(){if(!session?.access_token)return false;try{const response=await fetch("/api/generation-recover",{method:"POST",headers:{Accept:"application/json",Authorization:`Bearer ${session.access_token}`},cache:"no-store"});const data=await response.json().catch(()=>({}));return response.ok&&data?.restored===true;}catch(error){console.warn("OBITREND generation credit recovery skipped:",error);return false;}}
async function loadAccount(){if(!session?.access_token)return;try{const response=await fetch("/api/account",{method:"GET",headers:{Accept:"application/json",Authorization:`Bearer ${session.access_token}`},cache:"no-store"});const data=await response.json().catch(()=>({}));if(!response.ok||!data?.ok)throw new Error(data?.error||"Unable to load your account.");account=data.account;renderAccount();const restored=await recoverMalformedGenerationCredit();if(restored){const refreshed=await fetch("/api/account",{method:"GET",headers:{Accept:"application/json",Authorization:`Bearer ${session.access_token}`},cache:"no-store"});const refreshedData=await refreshed.json().catch(()=>({}));if(refreshed.ok&&refreshedData?.ok){account=refreshedData.account;renderAccount();toast("Your failed generation credit has been restored.");}}}catch(error){console.error("OBITREND account error:",error);account={user:{email:session.user?.email||"",obitrendUserId:""},imageCredits:{free:0,freeTotal:3,pro:0,proTotal:0,available:0},pro:{active:false,planName:null,credits:0},video:{seconds:0}};renderAccount();toast("Signed in. Account details are still loading.");}}
...
const showProgress=()=>{card?.classList.remove("hidden");ready?.classList.add("hidden");const sketchPreview=$("generationSketchPreview");if(sketchPreview){sketchPreview.style.aspectRatio="4 / 5";}setProgress(12,"Sketching it out",0);renderGenerationDots(12);clearInterval(progressTimer);clearInterval(generationDotTimer);let step=0;const steps=[[28,"Making the first draft",6],[44,"Setting the scene",5],[60,"Polishing details",4],[74,"Finishing up",3],[86,"Adding final touches",2],[96,"One last tweak...",1]];progressTimer=setInterval(()=>{if(step<steps.length){const s=steps[step++];setProgress(s[0],s[1],s[2]);renderGenerationDots(s[0]);}},1100);generationDotTimer=setInterval(()=>{const current=Number(percent?.textContent?.replace("%",""))||12;renderGenerationDots(current);},260);};
