...
function friendlyGenerationMessage(error){
  ...
  if(status===429&&/no credits remaining|insufficient_quota|credit_balance_exhausted|add credits to continue using the api/i.test(normalized))return "The AI image service is temporarily unavailable because its provider credits are exhausted. Your OBITREND image credit is restored automatically when no image is generated. Please try again after the image service is funded.";
...
const file=$("garmentInput")?.files?.[0]||$("creativeGarmentInput")?.files?.[0]||null;
...
finishProgress();
const generationPreview=$("generationSketchPreview");
if(generationPreview){
  generationPreview.classList.add("generation-result-preview");
  const oldResult=generationPreview.querySelector(".generation-result-image");
  if(oldResult)oldResult.remove();
  const resultImg=document.createElement("img");
  resultImg.className="generation-result-image";
  resultImg.alt="Generated OBITREND fashion image";
  resultImg.src=displayObjectUrl;
  generationPreview.replaceChildren(resultImg);
  generationPreview.setAttribute("aria-hidden","false");
  if(percent)percent.parentElement?.classList.add("hidden");
