...
function openPage(page){window.scrollTo({top:0,behavior:"instant"});const requested=page==="create"?"home":page;const target=requested==="settings"?"account":requested;const valid=["home","gallery","video","credits","account"];const safe=valid.includes(target)?target:"home";qsa(".page").forEach(el=>el.classList.remove("active-page"));const pageEl=$("page"+safe.charAt(0).toUpperCase()+safe.slice(1));if(pageEl)pageEl.classList.add("active-page");qsa(".nav-item").forEach(el=>el.classList.toggle("active",el.dataset.page===requested));const titles={home:"AI Fashion Creator",create:"Create",gallery:"Gallery",video:"Video AI",credits:"Credits",account:"Account",settings:"Settings"};$("pageTitle")&&($("pageTitle").textContent=titles[requested]||titles.home);$("pageKicker")&&($("pageKicker").textContent=requested==="home"?"OBITREND":requested.toUpperCase());closeSidebar();if(requested==="settings"){const heading=document.querySelector("#pageAccount .section-head");if(heading)setTimeout(()=>heading.scrollIntoView({behavior:"smooth",block:"start"}),0);}if((requested==="credits"||requested==="account")&&session?.access_token){loadAccount();}}function openSidebar(){const sidebar=$("sidebar"),overlay=$("overlay");if(sidebar)sidebar.classList.add("open");if(overlay){overlay.classList.add("open");overlay.style.display="block";overlay.style.pointerEvents="auto";}}function closeSidebar(){const sidebar=$("sidebar"),overlay=$("overlay");if(sidebar)sidebar.classList.remove("open");if(overlay){overlay.classList.remove("open");overlay.style.display="none";overlay.style.pointerEvents="none";}const dashboard=$("dashboardScreen");if(dashboard){dashboard.classList.remove("hidden");dashboard.style.display="block";}const active=document.querySelector(".page.active-page");if(!active){const home=$("pageHome");if(home)home.classList.add("active-page");}document.body.style.background="#08080b";}
