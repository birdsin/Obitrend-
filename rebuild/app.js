...
async function authAction(kind){const email=$("authEmail").value.trim().toLowerCase(),password=$("authPassword").value;if(!email||!email.includes("@"))return setAuthStatus("Enter a valid email address.","error");if(kind==="signin"&&restoreLoginControls(email))return;if(password.length<6)return setAuthStatus("Password must be at least 6 characters.","error");$("signInBtn").disabled=true;$("signUpBtn").disabled=true;setAuthStatus(kind==="signin"?"Signing in…":"Creating your account…");try{const result=kind==="signin"?await supabase.auth.signInWithPassword({email,password}):await supabase.auth.signUp({email,password});if(result.error)throw result.error;if(kind==="signup"&&!result.data.session){setAuthStatus("Account created. Check your email if confirmation is required, then sign in.","success");setAuthMode("signin");$("authPassword").value="";return;}if(kind==="signin")clearAuthAttempts(email);await handlePasswordRecoverySession();
loadSession();}catch(error){console.error("OBITREND auth error:",error);if(kind==="signin"&&/invalid login credentials|invalid credentials|invalid password/i.test(messageText(error))){const attempts=getAuthAttempts(email)+1;setAuthAttempts(email,attempts);if(attempts>=3){applyLoginBlock(email);setAuthStatus("Account sign-in is blocked after 3 incorrect passwords. Use Forgot password to reset access.","error");}else{setAuthStatus(`Incorrect password. ${3-attempts} attempt${3-attempts===1?"":"s"} remaining.`,"error");}}else setAuthStatus(safeMessage(error),"error");}finally{if(kind==="signin"){if(getAuthAttempts(email)<3)$("signInBtn").disabled=false;}else{$("signInBtn").disabled=false;$("signUpBtn").disabled=false;}}}
...
supabase.auth.onAuthStateChange((event,nextSession)=>{session=nextSession||null;if(event==="PASSWORD_RECOVERY"){showAuth();$("resetPasswordPanel")?.classList.remove("hidden");$("forgotBtn")?.classList.add("hidden");$("signInBtn")?.classList.add("hidden");$("signUpBtn")?.classList.add("hidden");setAuthStatus("Enter your new password.","success");return;}if(session){showDashboard();loadAccount().then(verifyReturnedPayment);}else showAuth();});
...
setupCreativeResultActions();setupImageCamera();setupReferenceAuth();setupReferenceDashboard();setupCreditOverlay();setupNotifications();
