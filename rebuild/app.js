...
const basePrompt=$("creativePrompt")?.value?.trim()||"Create a true-to-life professional AI fashion campaign using my uploaded clothing image. Preserve the garment design exactly.";
...
const prompt=[basePrompt,`Model/body: ${model}.`,`Background/location: ${background}.`,`Garment color direction: ${garmentColor}.`,`Trouser color direction: ${trouserColor}.`,"true-to-life professional fashion photography, full-body framing, realistic adult model, preserve uploaded garment design exactly."].join(" ");
...
input.addEventListener("change",()=>{const file=input.files?.[0];if(name)name.textContent=file?file.name:"image.jpg";if(file){const reader=new FileReader();reader.onload=()=>{input.dataset.preview=reader.result;const preview=$("dashboardGarmentPreview");if(preview){preview.src=reader.result;preview.style.display="block";preview.style.visibility="visible";}const size=$("dashboardImageSize");if(size)size.textContent=(file.size/(1024*1024)).toFixed(1)+" MB";const badge=$("dashboardImageBadge");if(badge)badge.innerHTML='Uploaded Image <i>✓</i>';};reader.readAsDataURL(file);toast("Clothing image ready.");}});
qsa(".ob-chip").forEach(b=>b.addEventListener("click",()=>{const p=$("creativePrompt");if(p)p.value=b.dataset.prompt||"";}));
qsa("[data-ob-upload]").forEach(b=>b.addEventListener("click",()=>input.click()));qsa("[data-ob-generate]").forEach(b=>b.addEventListener("click",handleCreative));button?.addEventListener("click",handleCreative);$("obProfileButton")?.addEventListener("click",()=>openPage("account"));$("obMenuButton")?.addEventListener("click",()=>{const menu=["home","create","gallery","video","credits","account"];const current=document.querySelector(".page.active-page")?.id?.replace(/^page/,"").toLowerCase()||"home";const next=menu[(menu.indexOf(current)+1)%menu.length];openPage(next);});renderRecentCreations();}
